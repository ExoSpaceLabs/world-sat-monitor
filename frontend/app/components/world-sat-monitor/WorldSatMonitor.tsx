...
import {SatelliteSettingsPanel} from "../satellite/SatelliteSettingsPanel";
...
import {DEFAULT_APP_SETTINGS, type AppSettings, type PersistentSatelliteSettings} from "../../domain/settings";
...
export function WorldSatMonitor() {
  ...
  const applyTimeScale = useCallback((scale: number) => {
    ...
    const simulationTimestamp = state.initialized ? state.simulationAnchorMs + (realTimestamp - state.realAnchorMs) * state.scale : realTimestamp;
    ...
    const nextNow = new Date(simulationTimestamp); nowRef.current = nextNow; setNow(nextNow);
  }, []);
  ...
  const scheduleSettingsSave = useCallback((next: AppSettings) => {
    ...
    settingsSaveTimerRef.current = window.setTimeout(() => {
      void saveAppSettings(next).then(() => { setPersistenceState("saved"); setApiState("online"); }).catch(() => { setPersistenceState("error"); setApiState("offline"); });
    }, SETTINGS_SAVE_DEBOUNCE_MS);
  }, []);
  const updateSettings = useCallback((next: AppSettings) => { setAppSettings(next); scheduleSettingsSave(next); }, [scheduleSettingsSave]);

  useEffect(() => () => { if (settingsSaveTimerRef.current !== null) window.clearTimeout(settingsSaveTimerRef.current); }, []);
  useEffect(() => {
    let cancelled = false;
    void getAppSettings().then((loaded) => {
      if (cancelled) return;
      setAppSettings(loaded); setBasemap(loaded.map.basemap);
      setScene({spaceEnvironment: loaded.map.space_environment, shadowOpacity: loaded.map.shadow_opacity, debug: loaded.map.debug});
      applyTimeScale(loaded.map.time_scale); setSettingsLoaded(true); setPersistenceState("saved"); setApiState("online");
    }).catch(() => { if (!cancelled) { setSettingsLoaded(true); setPersistenceState("error"); setApiState("offline"); } });
    return () => { cancelled = true; };
  }, [applyTimeScale]);

  useEffect(() => {
    const realNow = Date.now();
    if (!simulationClockRef.current.initialized) simulationClockRef.current = {initialized: true, realAnchorMs: realNow, simulationAnchorMs: realNow, scale: 1};
    const tick = () => {
      const state = simulationClockRef.current; const realTimestamp = Date.now();
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        const result = await getSatellitePosition(appSettings.satellite.selected_norad_id, nowRef.current);
        if (!cancelled) { setSatellite(result.satellite); setSatelliteIsMock(result.isMock); setPositionInterpolated(result.interpolated); setApiState("online"); }
      } catch { if (!cancelled) setApiState("offline"); } finally { inFlight = false; }
    };
    void refresh(); const interval = window.setInterval(refresh, appSettings.satellite.position_update_ms);
    return () => { cancelled = true; window.clearInterval(interval); };
  }, [appSettings.satellite.position_update_ms, appSettings.satellite.selected_norad_id, settingsLoaded]);

  useEffect(() => {
    if (!settingsLoaded) return;
    if (!appSettings.satellite.path.enabled || (appSettings.satellite.path.history_minutes === 0 && appSettings.satellite.path.prediction_hours === 0)) {
      setSatelliteTrack([]); setEffectivePathResolution(null); return;
    }
    let cancelled = false; let inFlight = false;
    const refresh = async () => {
      if (inFlight) return; inFlight = true;
      const center = nowRef.current;
      const start = new Date(center.getTime() - appSettings.satellite.path.history_minutes * 60_000);
      const end = new Date(center.getTime() + appSettings.satellite.path.prediction_hours * 3_600_000);
      try {
        const result = await getSatelliteTrack(appSettings.satellite.selected_norad_id, start, end, appSettings.satellite.path.resolution_seconds, center);
        if (!cancelled) { setSatelliteTrack(result.points); setEffectivePathResolution(result.resolutionSeconds); setSatelliteIsMock(result.isMock); setApiState("online"); }
      } catch { if (!cancelled) setApiState("offline"); } finally { inFlight = false; }
    };
    void refresh(); const interval = window.setInterval(refresh, appSettings.satellite.path.refresh_seconds * 1000);
    return () => { cancelled = true; window.clearInterval(interval); };
  }, [appSettings.satellite.path.enabled, appSettings.satellite.path.history_minutes, appSettings.satellite.path.prediction_hours, appSettings.satellite.path.refresh_seconds, appSettings.satellite.path.resolution_seconds, appSettings.satellite.selected_norad_id, settingsLoaded]);

  const solarState = useMemo(() => getSolarState(now), [now]);
  const persistMapSettings = useCallback((patch: Partial<AppSettings["map"]>) => { updateSettings({...appSettings, map: {...appSettings.map, ...patch}}); }, [appSettings, updateSettings]);
  ...
  const handleTimeReset = useCallback(() => { const realTimestamp = Date.now(); const scale = simulationClockRef.current.scale; simulationClockRef.current = {initialized: true, realAnchorMs: realTimestamp, simulationAnchorMs: realTimestamp, scale}; const nextNow = new Date(realTimestamp); nowRef.current = nextNow; setNow(nextNow); setTimeResetKey((key) => key + 1); }, []);
  ...
  const handleMapSettingsReset = useCallback(() => { const defaults = DEFAULT_APP_SETTINGS.map; setBasemap(defaults.basemap); setScene({spaceEnvironment: defaults.space_environment, shadowOpacity: defaults.shadow_opacity, debug: defaults.debug}); applyTimeScale(defaults.time_scale); handleTimeReset(); updateSettings({...appSettings, map: {...defaults}}); }, [appSettings, applyTimeScale, handleTimeReset, updateSettings]);
  const handleSatelliteSettingsChange = useCallback((next: PersistentSatelliteSettings) => { updateSettings({...appSettings, satellite: next}); }, [appSettings, updateSettings]);
  const handleSatelliteSettingsReset = useCallback(() => { updateSettings({...appSettings, satellite: {...DEFAULT_APP_SETTINGS.satellite, path: {...DEFAULT_APP_SETTINGS.satellite.path}}}); }, [appSettings, updateSettings]);
  const handleSatelliteSelect = useCallback(() => setFollowSatellite(true), []);

  const rotationLabel = followSatellite ? "CAMERA LOCKED TO SATELLITE · EARTH ROTATING" : rotationReason === "zoom" ? "CAMERA LOCKED TO EARTH ROTATION" : `${timeScale}× EARTH ROTATION ACTIVE`;
    ...
    <main className="monitor-shell" data-layer="page">
      <header className="topbar">
        ...
        <div className="topbar-actions">
          ...
          <button className={`settings-trigger ${satelliteSettingsOpen ? "active" : ""}`} onClick={() => { setSatelliteSettingsOpen((open) => !open); setMapSettingsOpen(false); }} aria-expanded={satelliteSettingsOpen} aria-label="Open satellite settings"><i/><span>SATELLITE SETTINGS</span></button>
          <button className={`settings-trigger ${mapSettingsOpen ? "active" : ""}`} onClick={() => { setMapSettingsOpen((open) => !open); setSatelliteSettingsOpen(false); }} aria-expanded={mapSettingsOpen} aria-label="Open map settings"><i/><span>MAP SETTINGS</span></button>
        </div>
      </header>
      <section className="viewport">
        ...
        <SatelliteLayer mapSession={mapSession} satellite={satellite} track={appSettings.satellite.path.enabled ? satelliteTrack : []} selected onSelect={handleSatelliteSelect}/>
        ...
        {satelliteSettingsOpen && <SatelliteSettingsPanel settings={appSettings.satellite} effectivePathResolution={effectivePathResolution} onChange={handleSatelliteSettingsChange} onReset={handleSatelliteSettingsReset} onClose={() => setSatelliteSettingsOpen(false)}/>} 
